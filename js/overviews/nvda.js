...
// Shared "Results" engine (Evolution ▸ Results / Estimates / Setup) — identical to AMZN's wiring.
import { resultsHtml, initResults, resultsEvoHtml, initResultsEvo } from '../results.js';
// Valuation sub-modules (Deep Dive ▸ Valuation), mirror amzn-histmult / amzn-target-multiple / amzn-sensitivity.
import { nvdaHistMult } from './nvda-histmult.js';
import { nvdaTargetMult } from './nvda-target-multiple.js';
import { nvdaSens } from './nvda-sensitivity.js';
import { nvdaManagement } from './nvda-management.js';
...
import { nvdaBottomLine } from './nvda-bottomline.js';
import { nvdaOverview2 } from './nvda-overview.js';
// Earnings sub-pane (Evolution ▸ Earnings) — the full AMZN-style Setup · Post-Results · Notes module,
// self-contained in its own file so it never conflicts with this one.
import { nvdaEarnings } from './nvda-earnings.js';

// Open company (id + ticker), captured in deepDiveHtml for the shared Watch List engine mount.
var _co = null;
...
// Overview = the AMZN-canonical standardized 7-block module (js/overviews/nvda-overview.js).
// The old box-based overviewBody()/initOverview() are retired (kept in-file as dead code).
function html(c){ return nvdaOverview2.body(c); }

function init(c){
  requestAnimationFrame(function(){
    var r = document.querySelector('.ov-nvda:not(.ov-nvda-dd)');
    nvdaOverview2.init(r || document);
  });
}
...
// ─── Deep Dive spine — AMZN-canonical: Top Line · Bottom Line · Evolution · Valuation
// · Management. (Miscellaneous is intentionally OMITTED for NVDA — no M&A/Capex deep-dive.)
// The Technology and Industry bodies from the old structure are retained in this file but no
// longer surfaced as top-level tabs; their roadmap facts fold into Top Line / Overview.

// Top Line — the two segment frameworks (recast Hyperscale/ACIE/Edge + old DC/Gaming/…) plus
// verbatim management commentary from the calls (docs/calls/NVDA.md).
// Top Line — the shared segments.js engine (format-identical to AMZN): General · Segments · Other ·
// Customers. Data comes from js/segments-data/nvda.js (registered in segments.js).
function toplineBody(){
  return '<div class="ovt-subtabs">'+
    ...
    '</div>'+
    '<div class="ovt-subpane" data-ovst="segov">'+(segmentsOverviewHtml('NVDA')||'')+'</div>'+
    '<div class="ovt-subpane" data-ovst="segdrv" hidden>'+(segmentsHtml('NVDA')||'')+'</div>'+
    '<div class="ovt-subpane" data-ovst="segoth" hidden>'+(segmentsOtherHtml('NVDA')||'')+'</div>'+
    '<div class="ovt-subpane" data-ovst="segcus" hidden>'+(segmentsCustomersHtml('NVDA')||'')+'</div>';
}
function tlBuild(root){
  var pane = root.querySelector('.ovt-pane[data-ovt="topline"]'); if (!pane) return;
  var sub = pane.querySelector('.ovt-subtab.active');
  var key = sub ? sub.getAttribute('data-ovst') : 'segov';
  if (key === 'segdrv')      requestAnimationFrame(function(){ initSegments(pane, 'NVDA'); });
  else if (key === 'segoth') requestAnimationFrame(function(){ initSegmentsOther(pane, 'NVDA'); });
  else if (key === 'segcus') requestAnimationFrame(function(){ initSegmentsCustomers(pane, 'NVDA'); });
  else                       requestAnimationFrame(function(){ initSegmentsOverview(pane, 'NVDA'); });
}

// Bottom Line — margins narrative + the gross-margin bridge (H20/China charge) + expense/opex
// commentary. Uses a summary table (no chart) to avoid colliding with the Overview margins
// canvas; the interactive margin trend lives in Overview and Evolution ▸ Results.
function blMarginRows(){
  var R = [['FY2024','72.7','54.1','48.9','58.6','52.1'],['FY2025','75.0','62.4','55.8','65.2','51.3'],
           ['FY2026','71.1','60.4','55.6','63.1','51.3'],['FY2027E','74.1','65.2','54.7','66.9','51.1']];
  return '<table class="ov-tbl"><thead><tr><th>Fiscal year</th><th>Gross</th><th>Operating</th><th>Net</th><th>EBITDA</th><th>FCF</th></tr></thead><tbody>'+
    R.map(function(r){ return '<tr><td>'+r[0]+'</td>'+r.slice(1).map(function(v){ return '<td style="text-align:right">'+v+'%</td>'; }).join('')+'</tr>'; }).join('')+
    '</tbody></table><p class="ov-note">GAAP reported (gross/operating/net) + Summit model (EBITDA/FCF), % of revenue. FY2027E from the Summit model.</p>';
}
function bottomlineBody(){
  return sec('Margins', blMarginRows() +
      '<p>FY2026 gross margin fell to 71.1% because Q1 FY2026 carried a ~$4.5B H20/China inventory charge; ex-charge and through the year it recovered toward ~75%. Management guides full-year gross margin to the <b>mid-70s</b>.</p>') +
    sec('The gross-margin bridge (H20 / China)',
      bullets([
        '<b>Q1 FY2026:</b> "Excluding the $4.5 billion charge, Q1 non-GAAP gross margins would have been 71.3%." — Colette Kress',
        '<b>Q1 FY2027:</b> "GAAP gross margin was 74.9% and non-GAAP gross margins was 75%, largely flat sequentially." — Colette Kress',
        '<b>The lever (Q4 FY2026):</b> "The single most important lever of our gross margins is actually delivering generational leaps to our customers." — Jensen Huang'
      ])) +
    sec('Operating expenses, SBC &amp; tax',
      bullets([
        '<b>Opex (Q1 FY2027):</b> "GAAP and non-GAAP operating expenses were up 12% sequentially, primarily due to higher compensation and an increase in compute and infrastructure costs… For the full year, we now expect OPEX to grow somewhere in the upper 40s on a year-over-year basis." — Colette Kress',
        '<b>SBC (Q4 FY2026):</b> "Starting this quarter, we will be including stock-based compensation expense in our non-GAAP results." — Colette Kress',
        '<b>Tax (Q1 FY2027):</b> "Our non-GAAP effective tax rate of 16% came just below our prior outlook due to favorable geographic mix… For the full year 2027, we expect GAAP and non-GAAP tax rates to be between 16% and 18%." — Colette Kress'
      ]));
}

// Evolution — nested sub-tabs (Earnings · Results · Estimates), mirroring AMZN. Earnings carries
// the Setup dataset (results engine) + the consensus/guidance-beats analysis (old consensusBody).
// Results/Estimates are the shared results.js engine against nvdaResults / its evolution block.
function evolutionBody(){
  return '<div class="ce-evohead"><div class="ovt-subtabs">'+
      ...
      '<button type="button" class="ovt-subtab" data-ovst="estimates">Estimates</button>'+
    '</div></div>'+
    // Earnings = the full AMZN-style phase module (Setup · Post-Results · Notes), self-contained in
    // js/overviews/nvda-earnings.js. It owns the Setup chart (NVDA_SETUP), the authored Post-Results,
    // the shared Watch List mount and the theme record. (The old consensusBody guidance-beats view is
    // still in this file but no longer surfaced here.)
    '<div class="ovt-subpane" data-ovst="earnings">'+ nvdaEarnings.body(_co) +'</div>'+
    '<div class="ovt-subpane" data-ovst="results" hidden>'+resultsHtml('NVDA')+'</div>'+
    '<div class="ovt-subpane" data-ovst="estimates" hidden>'+resultsEvoHtml('NVDA')+'</div>';
}

// Valuation — sub-tabs Calculator (old valuationBody) + Historic Multiple / Target Multiple·PEG /
// Sensitivity (the imported sibling modules).
function valuationTabBody(){
  return '<div class="ovt-subtabs">'+
    ...
    '</div>'+
    '<div class="ovt-subpane" data-ovst="histmult">'+nvdaHistMult.body()+'</div>'+
    '<div class="ovt-subpane" data-ovst="peers" hidden>'+
      '<p class="ov-lede">NVIDIA against the other AI-silicon names on valuation and growth — bubble size is live market cap; the comps table sits below.</p>'+
      nvdaScatterHtml('dd')+'</div>'+
    '<div class="ovt-subpane" data-ovst="targetmult" hidden>'+nvdaTargetMult.body()+'</div>'+
    '<div class="ovt-subpane" data-ovst="sensitivity" hidden>'+nvdaSens.body()+'</div>';
}

var DEEP_TABS = [
  { key:'topline',    label:'Top Line',    body: toplineBody },
  { key:'bottomline', label:'Bottom Line', body: nvdaBottomLine.body },
  { key:'evolution',  label:'Evolution',   body: evolutionBody },
  { key:'valuation',  label:'Valuation',   body: valuationTabBody },
  { key:'mgmt',       label:'Management',   body: nvdaManagement.body },
];

function deepDiveHtml(c){
  _co = c || _co;   // capture company (id + ticker) for the Earnings Watch List mount
...
// Build the chart(s) that belong to a given top-level Deep-Dive tab (render-on-reveal).
function ddBuild(root, key){
  stopTech();
  if (key === 'topline')    { requestAnimationFrame(function(){ tlBuild(root); }); }
  else if (key === 'evolution') { requestAnimationFrame(function(){ evoBuild(root); }); }
  else if (key === 'valuation') { requestAnimationFrame(function(){ valBuild(root); }); }
  else if (key === 'mgmt')  { var mp = root.querySelector('.ovt-pane[data-ovt="mgmt"]'); requestAnimationFrame(function(){ nvdaManagement.init(mp || root); }); }
  else if (key === 'bottomline') { var bp = root.querySelector('.ovt-pane[data-ovt="bottomline"]'); requestAnimationFrame(function(){ nvdaBottomLine.init(bp || root); }); }
}

function showOvt(root, key){
  root.querySelectorAll(':scope > .ovt-tabs > .ovt-tab').forEach(function(b){ b.classList.toggle('active', b.getAttribute('data-ovt') === key); });
  root.querySelectorAll(':scope > .ovt-pane').forEach(function(p){ p.hidden = (p.getAttribute('data-ovt') !== key); });
  ddBuild(root, key);
}

// Evolution sub-tab wiring: Earnings → Setup dataset + consensus charts; Results → nvdaResults;
// Estimates → the evolution block. Each builds only when its sub-pane is revealed.
function evoBuild(root){
  var pane = root.querySelector('.ovt-pane[data-ovt="evolution"]'); if (!pane) return;
  var sub = pane.querySelector('.ovt-subtab.active');
  var key = sub ? sub.getAttribute('data-ovst') : 'earnings';
  var earn = pane.querySelector('.ovt-subpane[data-ovst="earnings"]');
  var res  = pane.querySelector('.ovt-subpane[data-ovst="results"]');
  if (key === 'earnings') { if (earn) nvdaEarnings.init(earn, _co); }
  else if (key === 'results')  { if (res) initResults(res, 'NVDA'); }
  else if (key === 'estimates'){ initResultsEvo('NVDA'); }
}
function valBuild(root){
  var pane = root.querySelector('.ovt-pane[data-ovt="valuation"]'); if (!pane) return;
  var sub = pane.querySelector('.ovt-subtab.active');
  var key = sub ? sub.getAttribute('data-ovst') : 'histmult';
  if (key === 'peers') requestAnimationFrame(function(){ wireNvdaScatters(pane); });
  else if (key === 'targetmult' && nvdaTargetMult.init) requestAnimationFrame(function(){ nvdaTargetMult.init(pane); });
  else if (key === 'sensitivity' && nvdaSens.init) requestAnimationFrame(function(){ nvdaSens.init(pane); });
  else if (nvdaHistMult.init) requestAnimationFrame(function(){ nvdaHistMult.init(pane); });
}

// Generic sub-tab wiring for panes that carry an inner .ovt-subtabs bar (Evolution, Valuation).
// Pane-scoped so Evolution's and Valuation's identical machinery never collide.
function wireSubtabs(pane, onShow){
  var bar = pane.querySelector('.ovt-subtabs'); if (!bar) return;
  bar.querySelectorAll('.ovt-subtab').forEach(function(btn){
    btn.onclick = function(){
      var k = btn.getAttribute('data-ovst');
      bar.querySelectorAll('.ovt-subtab').forEach(function(x){ x.classList.toggle('active', x===btn); });
      pane.querySelectorAll(':scope > .ovt-subpane').forEach(function(p){ p.hidden = (p.getAttribute('data-ovst') !== k); });
      requestAnimationFrame(function(){ onShow(); });
    };
  });
}

// Idempotent: called every time the Deep Dive tab becomes visible.
function deepDiveInit(){
  var root = document.querySelector('.ov-nvda-dd');
  if (!root) return;
  root.querySelectorAll(':scope > .ovt-tabs > .ovt-tab').forEach(function(btn){
    btn.onclick = function(){ showOvt(root, btn.getAttribute('data-ovt')); };
  });
  // Top Line: segment / view / forecast-source toggles on the revenue chart.
  ...
  // Nested sub-tab bars.
  var evoPane = root.querySelector('.ovt-pane[data-ovt="evolution"]');
  if (evoPane) wireSubtabs(evoPane, function(){ evoBuild(root); });
  var valPane = root.querySelector('.ovt-pane[data-ovt="valuation"]');
  if (valPane) wireSubtabs(valPane, function(){ valBuild(root); });
  var tlPane = root.querySelector('.ovt-pane[data-ovt="topline"]');
  if (tlPane) wireSubtabs(tlPane, function(){ tlBuild(root); });
  var active = root.querySelector(':scope > .ovt-tabs > .ovt-tab.active');
  var key = active ? active.getAttribute('data-ovt') : 'topline';
  requestAnimationFrame(function(){ showOvt(root, key); });
}

// IR + EDGAR source tiles for the profile header (#co-srcbtns), mirroring amzn.js ceHeaderSources.
function nvdaHeaderSources(){
  var IR='https://investor.nvidia.com/', EDGAR='https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0001045810&type=10-K&dateb=&owner=include&count=40';
  var LOGO='https://assets.parqet.com/logos/symbol/NVDA', SEAL='img/sec-seal.png';
  return '<style>.cohd-src{display:inline-flex;gap:8px;align-items:center}'+
    '.cohd-src a{width:42px;height:42px;border-radius:10px;display:flex;align-items:center;justify-content:center;text-decoration:none;position:relative;overflow:hidden;transition:.16s;'+
      'background:linear-gradient(135deg,#04140A 0%,#0A2410 60%,#04140A 100%);border:1px solid rgba(118,185,0,.40);box-shadow:0 3px 12px rgba(0,0,0,.32)}'+
    '.cohd-src a:hover{transform:translateY(-1px);box-shadow:0 8px 20px rgba(118,185,0,.30);border-color:rgba(118,185,0,.82)}'+
    ...
    '.cohd-src a.edgar img{border-radius:0}</style>'+
    '<div class="cohd-src">'+
    '<a href="'+IR+'" target="_blank" rel="noopener" title="NVIDIA Investor Relations" aria-label="NVIDIA Investor Relations"><img src="'+LOGO+'" alt="NVIDIA logo" onerror="this.style.display=\'none\'"></a>'+
    '<a class="edgar" href="'+EDGAR+'" target="_blank" rel="noopener" title="NVIDIA on SEC EDGAR" aria-label="NVIDIA on SEC EDGAR"><img src="'+SEAL+'" alt="SEC seal" onerror="this.style.display=\'none\'"></a>'+
    '</div>';
}

export var nvdaOverview = {
  html: html,
  init: init,
  headerSources: nvdaHeaderSources,
