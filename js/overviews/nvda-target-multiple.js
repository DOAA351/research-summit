// overviews/nvda-target-multiple.js — NVIDIA (NVDA) Deep Dive ▸ Valuation ▸ Target Multiple / PEG.
//
// Structural twin of overviews/amzn-target-multiple.js — same export shape, same controls, same
// §0 standard, same PEG block. Only the data and the identity differ. Built around the desk
// workflow: a forward multiple is chosen — BOTH EV/EBITDA and P/E — on the working year (FY2027),
// giving a year-end price target; the revision log records which was chosen.
//
...
//
// The x axis is SNAPSHOTS, not periods, so onX is null on the brush (only the y axis zooms) and
// rsFwdZone is not used. Every summary figure is computed over the selected window (invariant 9).
//
// ── Data (Summit DCF model, instrument NVDA, FY2027 / FY2028) ─────────────────────
// NVDA revenue · EBITDA · net income · diluted shares from the Summit model, per FWD year. Real
// anchors supplied: FY2027 rev 390,324 / EBITDA 261,030 / NI 213,316; FY2028 rev 566,719 /
// EBITDA 379,650 / NI 310,280 ($M); diluted shares 24,390M; net cash $109.2B.
//
// ── Only ONE vintage is available for NVDA ───────────────────────────────────────
// The AMZN twin carries seven dated snapshots (a real revision history). For NVDA only the current
// model vintage was supplied, so SNAP_DATA holds a SINGLE snapshot per forward year, clearly
// labelled "current vintage". With one row the change columns (Δ) and "vs realized" are empty by
// construction — nothing is fabricated to fill them. When the revision history is wired (one file
// per revision, as in the AMZN build), add the rows and everything downstream lights up unchanged.
//
// ── No EBITDA-redefinition break for NVDA ────────────────────────────────────────
// BREAK_ON is set to a sentinel that matches no snapshot, so no column is badged and FWD_NOTE is
// null on both years — there is no NVDA definition break in the supplied data.
//
// ── What is still missing ────────────────────────────────────────────────────────
// The recorded/realized multiples (past column EM on projection_history) and the FY2026 prior-year
// EBITDA/earnings behind the FY2027 PEG were not supplied. They render as stated absences (purple
// stand-in / "no prior-year figure"), never as invented numbers.
...
var BREAK_ON = '__none__';                 // sentinel: no NVDA EBITDA-redefinition break in the data

// $M / shares in millions, from the Summit model (NVDA revenue · EBITDA · net income · diluted
// shares). The forward year rolls: FY2027 is this year's working year, FY2028 is next year's.
// Only the current model vintage was supplied, so each year holds ONE snapshot — add rows here
// when the revision history is wired and every downstream column lights up unchanged.
var SNAP_DATA = {
  2027: [
    { d:'2026-08-04', rev:390324, ebitda:261030, earn:213316, sh:24390 },   // current vintage
  ],
  2028: [
    { d:'2026-08-04', rev:566719, ebitda:379650, earn:310280, sh:24390 },   // current vintage
  ],
};
...
// FY2026 (the prior year behind the FY2027 PEG). Only prior-year REVENUE was supplied (215,938 $M);
// FY2026 EBITDA and earnings were not, so they are left null and the FY2027 PEG reads as a stated
// absence ("no prior-year figure") rather than being fabricated. FY2028's prior is FY2027, which
// the table already holds.
var SNAP_2026 = [
  { d:'2026-08-04', rev:215938, ebitda:null, earn:null },   // current vintage; only rev supplied
];
...
// No EBITDA-redefinition break in the supplied NVDA data, so neither forward year carries a note.
var FWD_NOTE = {
  2027: null,
  2028: null,
};
...
// Illustrative only — a recorded multiple was NOT supplied for NVDA, so the stand-in is just the
// default config multiple held flat across the (single) snapshot. Purple-marked wherever it shows.
var MOCK_EV = [22];   // EV/EBITDA default (config: min 8 · default 22 · max 45)
var MOCK_PE = [33];   // P/E default (config: min 10 · default 33 · max 60)
...
var _mEv  = 22;           // EV/EBITDA default (config 8 / 22 / 45)
var _mPe  = 33;           // P/E default (config 10 / 33 / 60)
...
// No realized multiple was supplied for NVDA, so both years are pending — the "vs realized" column
// stays a stated absence rather than being marked against an invented number.
var REALIZED = {
  2027: { at:'31 Dec 2026', ev:null, pe:null },
  2028: { at:'31 Dec 2027', ev:null, pe:null },
};
...
function footHtml(){
  ...
  var common='FY'+FWD_YEAR+' revenue, EBITDA, net income and diluted share count from the Summit DCF model (instrument NVDA). '+
    '<b>Year-end target</b> = EBITDA × the EV/EBITDA multiple less net debt over shares, and EPS × the P/E multiple — '+
    'both shown side by side because the desk sets both. The <b>spread</b> is the disagreement between the two methods. Share count is the model\'s own '+
    '(24,390M diluted). Net cash/debt is '+fmtB(_netDebt)+' (net cash $109.2B), from the live quote. '+
    '<b>⚑ Only the current model vintage was supplied for NVDA</b>, so the log holds one snapshot per forward year — the change (Δ) columns are empty '+
    'until the revision history is wired. '+
...
export var nvdaTargetMult = { body: tmBody, init: initTm };
